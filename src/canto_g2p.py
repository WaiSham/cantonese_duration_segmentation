...
pron = {l.split('\t')[0]:l.split('\t')[1].strip() for l in open(os.path.join("out", "dictionary", "yue.tsv"), "r", encoding="utf-8").readlines()}

# check if output file exist, create a new file name with incremented number
if os.path.exists(os.path.join("out", "dictionary", "lexicon.txt")):
    base, ext = os.path.splitext(os.path.join("out", "dictionary", "lexicon.txt"))
    i = 1
    while os.path.exists(f"{base}_{i}{ext}"):
        i += 1
    output_file = f"{base}_{i}{ext}"
else:
    output_file = os.path.join("out", "dictionary", "lexicon.txt")
...
with open(output_file, "w", encoding="utf-8") as output:
    ...
    rows=[]
    with open(os.path.join("out", "dictionary", "words.txt"), "r", encoding="utf-8") as f:
        for line in tqdm(f):
            w = line.strip()
            word_pron = ''
            if w in pron:
